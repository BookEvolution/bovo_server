...
import com.bovo.Bovo.modules.chatrooms.dto.request.ChatMessageRequestDto;
...
public class LiveChatController {
    ...
    public void sendMessage(@DestinationVariable Integer roomId, ChatMessageRequestDto message) {
        log.info("받은 메시지: {}", message);

        Optional<Users> userOptional = usersRepository.findByEmail(message.getUsers().getEmail());
        if (userOptional.isEmpty()) {
            log.error("email을 찾을 수 없습니다. ID: {}", message.getUsers().getEmail());
            return;
        }
